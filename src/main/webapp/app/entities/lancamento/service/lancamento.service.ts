...
type RestOf<T extends ILancamento | NewLancamento> = Omit<T, 'dtLancamento'> & {
...
export class LancamentoService {
  ...
  protected convertDateFromClient<T extends ILancamento | NewLancamento | PartialUpdateLancamento>(lancamento: T): RestOf<T> {
    return {
      ...lancamento,
      dtLancamento: lancamento.dtLancamento?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restLancamento: RestLancamento): ILancamento {
    return {
      ...restLancamento,
      dtLancamento: restLancamento.dtLancamento ? dayjs(restLancamento.dtLancamento) : undefined,
    };
  }

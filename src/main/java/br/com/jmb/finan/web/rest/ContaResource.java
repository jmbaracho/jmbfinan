...
/**
 * REST controller for managing {@link br.com.jmb.finan.domain.Conta}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ContaResource {
    ...
    public ResponseEntity<Conta> partialUpdateConta(@PathVariable(value = "id", required = false) final Long id, @RequestBody Conta conta)
        ...
        Optional<Conta> result = contaRepository
            ...
            .map(existingConta -> {
                if (conta.getDsConta() != null) {
                    existingConta.setDsConta(conta.getDsConta());
                }
                if (conta.getVlSaldo() != null) {
                    existingConta.setVlSaldo(conta.getVlSaldo());
                }
